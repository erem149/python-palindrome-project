...
def palindrome():
    ...
    for num1 in range(low_val, high_val):
        for num2 in range(low_val, high_val):
            ...
            if num1 == num2:
                break
    print('print of palindromes:\n', palindromes_list, num1, num2)
    print('debug list:\n', debug_list)
    print('Iterations:\n', iterations)
    ...
    for dig1 in range(10):
        for dig2 in range(10):
            for dig3 in range(10):
                ...
                high_val = int(palindrome**0.5) + 1 # this **0.5 is "the square root" of palindrome

                # check if palindrome divisible by any of the numbers between min and max
                for digit in range(low_val, high_val):
                    iterations += 1
                    if palindrome % digit == 0: # check for divisibility, since we are stepping
                        #through palindromes in order, as soon as we find one: we are Done!
                            return  'create - palindrome:', palindrome, 'digit',digit, 'palindrome/digit', palindrome/digit, 'iterations', iterations
                            #gives nicer close out of loops
        ...
        for dig2 in range(9, -1, -1):
            for dig3 in range(9, -1, -1):
                ...
                for digit in range(low_val, high_val):
                    ...
                    if palindrome % digit == 0: # check for divisibility, since we are stepping
                        #through palindromes in order, as soon as we find one: we are Done!
                            return  'create2 - palindrome:', palindrome, 'digit',digit, 'palindrome/digit', palindrome/digit, 'iterations', iterations
